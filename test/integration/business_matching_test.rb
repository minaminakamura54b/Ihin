...
class BusinessMatchingTest < ActiveSupport::TestCase
  ...
  test "user が nil の場合はエリアフィルターなしで全業者を返す（最大3社）" do
    create(:business, service_prefectures: %w[東京都], plan: :free)
    create(:business, :premium_plan,  service_prefectures: %w[大阪府])
    create(:business, :standard_plan, service_prefectures: %w[神奈川県])
  ...
  test "ユーザーの都道府県にマッチする業者のみ返す" do
    ...
    tokyo_biz  = create(:business, service_prefectures: %w[東京都 神奈川県])
  ...
  test "prefecture が nil の場合はエリアフィルターをスキップする" do
    ...
    biz1 = create(:business, service_prefectures: %w[東京都])
    biz2 = create(:business, service_prefectures: %w[大阪府])
  ...
  test "カテゴリでフィルターできる" do
    user       = create(:user)
    estate_biz  = create(:business, category: :estate_clearance, service_prefectures: %w[東京都])
    buyback_biz = create(:business, category: :buyback,          service_prefectures: %w[東京都])
  ...
  test "最大3社に絞られる" do
    user = create(:user)
    4.times { create(:business, service_prefectures: %w[東京都]) }
  ...
  test "プレミアム業者が先頭に来る（プラン順）" do
    user         = create(:user)
    free_biz     = create(:business,               service_prefectures: %w[東京都])
    premium_biz  = create(:business, :premium_plan,  service_prefectures: %w[東京都])
    standard_biz = create(:business, :standard_plan, service_prefectures: %w[東京都])
    ...
    assert premium_idx < free_idx, "premium が free より先に来るべき"
  end

  test "inactive な業者は結果に含まれない" do
    user         = create(:user)
    active_biz   = create(:business,           service_prefectures: %w[東京都])
    inactive_biz = create(:business, :inactive, service_prefectures: %w[東京都])
